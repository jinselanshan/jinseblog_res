...
(function() {
	...
	var U = [
			...
			"as,base,by,checked,decimal,delegate,descending,dynamic,event,"
					...
					+ "var,virtual,where" ];
	var r = "all,and,by,catch,class,else,extends,false,finally,"
	...
	var x = [
			...
			"debugger,eval,export,function,get,null,set,undefined,var,with,"
					+ "Infinity,NaN" ];
	var s = "caller,delete,die,do,dump,elsif,eval,exit,foreach,for,"
	...
	function k(ac) {
		...
		for (var Y = 0, X = ac.length; Y < X; ++Y) {
			...
			if (ah.ignoreCase) {
				...
				if (/[a-z]/i.test(ah.source.replace(
		...
		function aa(ao) {
			var at = ao.substring(1, ao.length - 1).match(
			...
			for (var au = aq ? 1 : 0, am = at.length; au < am; ++au) {
				...
				if (/\\[bdsw]/i.test(ak)) {
					...
					if (!(an < 65 || aj > 122)) {
						if (!(an < 65 || aj > 90)) {
							ai.push([ Math.max(65, aj) | 32,
									Math.min(an, 90) | 32 ])
						}
						if (!(an < 97 || aj > 122)) {
							ai.push([ Math.max(97, aj) & ~32,
									Math.min(an, 122) & ~32 ])
						}
					}
				}
		...
		function Z(ao) {
			var am = ao.source.match(new RegExp("(?:"
					+ "\\[(?:[^\\x5C\\x5D]|\\\\[\\s\\S])*\\]"
	...
	function o(V) {
		...
		for (var X = V.firstChild; X; X = X.nextSibling) {
			var W = X.nodeType;
			Y = (W === 1) ? (Y ? V : X) : (W === 3) ? (v.test(X.nodeValue) ? V
					: Y) : Y
		}
	...
	function h(af) {
		...
		ab.push([ I, /^@[a-z_$][a-z_$@0-9]*/i, null ], [ R,
				/^(?:[@_]?[A-Z]+[a-z][A-Za-z_$@0-9]*|\w+_t\b)/, null ], [ H,
				/^[a-z_$][a-z_$@0-9]*/i, null ], [
				I,
				new RegExp("^(?:" + "0x[a-f0-9]+"
						+ "|(?:\\d(?:_\\d+)*\\d*(?:\\.\\d*)?|\\.\\d\\+)"
						+ "(?:e[+\\-]?\\d+)?" + ")" + "[a-z]*", "i"), null,
				"0123456789" ], [ H, /^\\[\s\S]?/, null ], [ N, new RegExp(V),
				null ]);
		return f(X, ab)
	}
	var M = h({
				...
				if ((an == 3 || an == 4) && ab) {
					...
					if (al) {
						...
						if (aj) {
							...
							am.insertBefore(ae.createTextNode(aj),
									ao.nextSibling)
						}
						af(ao);
						if (!ak) {
		...
		try {
			...
			while (ak < W) {
				...
				if (an.nodeType !== 1 && (X = ao.substring(Z, aa))) {
					...
					if (Z < V) {
						ae[ak + 1] = an = am
								.createTextNode(ao.substring(aa, V));
	...
	function w(al, ab) {
		...
		var am = {};
		function af() {
			var ay = (P["PR_SHOULD_USE_CONTINUATION"] ? ap["now"]() + 250
					: Infinity);
			for (; ai < ae.length && ap["now"]() < ay; ai++) {
				...
				for (var ax = aA; (ax = ax.previousSibling);) {
					...
					if (aF ? !/^\??prettify\b/.test(aF) : (aE !== 3 || /\S/
							.test(ax.nodeValue))) {
						break
					}
					if (aF) {
				...
				if ((aH !== am || an.test(aB)) && !W.test(aB)) {
					...
					if (!aD) {
						...
						if (Z.test(aA.tagName)) {
							...
							var aq = (at ? at["whiteSpace"]
									: (az && az.getComputedStyle) ? az
						...
						if (!(aC = aC === "true" || +aC)) {
							...
							aC = aC ? aC[1] && aC[1].length ? +aC[1] : true
									: false
						}
						if (aC) {
							T(aA, aC, av)
	...
	var a = P["PR"] = {
		...
		"PR_TAG" : m,
		"PR_TYPE" : R,
		"prettyPrintOne" : IN_GLOBAL_SCOPE ? (P["prettyPrintOne"] = A)
				: (prettyPrintOne = A),
		"prettyPrint" : prettyPrint = IN_GLOBAL_SCOPE ? (P["prettyPrint"] = w)
				: (prettyPrint = w)
	};
